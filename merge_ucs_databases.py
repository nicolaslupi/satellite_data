...
def str2long(str):
  if re.search('\d', str) is None:
    """ Doesn't Contain Longitude """
    return np.nan
  else:
    """ Contains Longitude Data """
    longitude = re.split(pattern=' (?=\d)', string=str)[1]
...
def simplify_users(str):
  if re.search( pattern = 'mil', string = str.lower() ) is not None:
    return 'Military'
  if re.search( pattern = 'gov', string = str.lower() ) is not None:
    return 'Government'
  if re.search( pattern = 'com', string = str.lower() ) is not None:
    return 'Commercial'
  if re.search( pattern = 'civ', string = str.lower() ) is not None:
    return 'Civil'
  return str
...
def process_database(database):
  ...
  if 'Class of Orbit' in database.columns: # Old Version of Database
    pass
...
data = pd.concat(reverse_databases, ignore_index=True)
